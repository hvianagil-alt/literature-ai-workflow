...
When you run the real workflow on your own papers in `papers/`, your outputs will land in `review/table/literature-table.md`, then `review/runs/<id>/synthesis-rationale.md`, then — after targeted extra retrieval and a form check against [`review/memory/`](../review/memory/README.md) — `review/runs/<id>/article.md`. Table, then interpretation and gap-fill, then the article, then `scripts/check_article.py` and `scripts/check_harness.py --full`.
