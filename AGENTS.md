...
**Abbreviations.** Terms that repeat (about five times or more) are written out once, then the short form: `type 2 diabetes (T2D)`, then `T2D`. See `review-prose`. Do not invent abbreviations, and do not keep spelling the long form after it has been defined.
