...
`check_article.py` fails a too-short Introduction, a missing aim paragraph, a generic Results heading, too few thematic `##` sections, **or an Introduction/first theme that explains mechanisms as unjoined short sentences**. Passing the 6,000-word floor is not enough. The double-check must record an adjacent-field reader test **and** a mechanism-articulation test; if either fails, rewrite without asking the user.
