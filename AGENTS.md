...
1. Write `review/runs/<run-id>/synthesis-rationale.md` (fallback `review/report/synthesis-rationale.md` or `review/notes/_synthesis-rationale.md`). Interpret the **whole sample**. The file **must** state: **(i)** the intellectual model (question, variables, mechanisms, heterogeneity, evidence types, established / uncertain / unknown, consequence); **(a)** what each included study actually measured; **(b)** themes the data support vs themes that would be forced; **(c)** real disagreements and why (methods / population / endpoint); **(d)** what this sample cannot answer; **(e)** the outline of the review; **(j)** **condition clusters** (group papers that share an endpoint family; name what is the same and what differs — population, cells or strain, dose, endpoint, geography, statistics — before any article prose). **No article yet.** If you cannot write (i) from the table, you are not ready to write the article (`scientific-synthesis`). If you cannot write (j), the article will become a catalogue. If the user says “just write the review,” still write this file first, then the article, and tell them that you did.
...
`check_article.py` fails a too-short Introduction, a missing aim paragraph, a generic Results heading, too few thematic `##` sections, **an Introduction/first theme that explains mechanisms as unjoined short sentences**, **or thematic sections that dump one study per paragraph without naming why similar experiments agree or differ**. Passing the 6,000-word floor is not enough. The double-check must record an adjacent-field reader test, **a mechanism-articulation test, and a condition-cluster test**; if any fails, rewrite without asking the user.
...
**References.** Number citations in **first-appearance order**: the first paper cited in the body is `[1]`, the next new paper is `[2]`, and so on. The list at the end is that sequence. Put a **blank line between each reference** (one paragraph per entry). After drafting, run `python3 scripts/renumber_citations.py --article review/runs/<run-id>/article.md`. `check_article.py` fails if the numbers skip or if the list is stacked without paragraph breaks.
