...
`check_article.py` still cannot judge every inference. It **does** fail conversational review-metaphor, stacked *According to Author et al.*, consecutive one-study result dumps (condition clusters), and the usual flourish/process bans. `check_review_craft.py` fails a flat body, meta-reviewer diction, slogan repetition, and missing CARS. The double-check must catch a paragraph that cites two papers but still never names the variable that changed. Do not tell the user the article is done while any of those fail.
