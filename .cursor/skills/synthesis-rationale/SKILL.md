...
Only after **Decision to write** is filled: use the `report-writing` skill. That skill must read this file and use section **(e)** as the article outline. The article must be a PhD-quality, argument-driven review of **all** in-scope evidence (original sample **plus** successfully retrieved gap-fill papers), not a catalogue of abstracts. Remaining open gaps must appear in the article as open gaps, not as silent omissions. Do not put token estimates in this file or in the article.
