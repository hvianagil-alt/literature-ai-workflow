...
`check_extraction.py`, `check_article.py`, and `check_review_craft.py` catch stubs, catalog voice, missing tables, citations in the Abstract, and a flat paper-catalogue body. They do **not** catch a wrong number, a DRAFT literature table, or a note that still only contains the PDF lead. This step is that second look. Do not tell the user the review is done until it is finished.
...
9. **Story spine (do not skip; do not wait for the user).** Adjacent-field reader test: after the Introduction alone, can someone expert in a neighbouring field follow the later sections? Are `##` headings thematic arguments, not a generic Results dump plus fragment `###` notes? **Mechanism test:** do teaching paragraphs join condition → effect → measurement, graded by evidence? **Condition-cluster test:** do thematic sections gather similar experiments and name why they agree or differ (population, cells or strain, dose, endpoint, geography, statistics), or is each paragraph still one study and one percentage? **Synthesis test (`scientific-synthesis`):** can you name each paragraph’s question; is the Discussion a “so what” rather than a second Results; can the whole paper be stated in five steps (problem, evidence, explanation, limitation, contribution)? **Published-review craft (`review-writing-craft`):** nested 3.1 topics on mechanisms (not only Methods 2.1); hinge vs grouped supporting papers; CARS niche before the aim; no meta-reviewer diction. If any answer is no, rewrite `article.md` now and re-run `check_article.py` and `check_review_craft.py`. Record the tests in the log.
10. **Discussion.** Interprets findings. No identification counts, fetch logs, or token meters.
11. **Write the log** to `review/runs/<run-id>/double-check.md` (or `review/report/double-check.md`). Also write `critic-log.md` with `Critic verdict: PASS` or `FAIL` (see `.cursor/agents/literature-critic.md`). Prefer launching that critic as a separate agent after the scripts pass.

```markdown
# Double-check — <run-id>

- Scripts: check_extraction.py … ; check_article.py … ; check_review_craft.py …
...
- Condition clusters (similar experiments compared; differences named): yes/no. If no, rewrite before delivering.
- Nested 3.1 topics / hinge vs supporting papers (`review-writing-craft`): yes/no.
...
- Field memory card: `review/memory/…`
...
After the log exists, remaining fixes are in `article.md`, **and** `python3 scripts/check_harness.py --run-dir review/runs/<id> --full` exits 0, tell the user the article is ready. **Ask in that same message if they also want Word and PDF.** Offer to iterate (add papers, adjust scope, refine a section).
